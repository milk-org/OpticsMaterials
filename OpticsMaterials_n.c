...
const struct MaterialIndex MatCode[] = {
    {"Mirror", 0}, {"SiO2", 1}, {"Si", 2},  {"PMGI", 3},     {"PMMA", 4},
    {"N2", 5},     {"O2", 6},   {"Ar", 7},  {"He", 8},       {"H2", 9},
    {"H2O", 10},   {"CO2", 11}, {"Ne", 12}, {"O", 13},       {"CaF2", 14},
    {"CH4", 16},   {"Kr", 17},  {"O3", 18}, {"UNKNOWN", 99}, {"Vacuum", 100},
    {"Air", 101},  {NULL, 0} /* end marker */
};
...
double OpticsMaterials_n(int material, double lambda)
{
    ...
    case 101: // air, 1 atm
        n = 1.0 +
            1.0 * ((5792105.0e-8 / (238.0185 - 1.0 / (lambdaum * lambdaum))) +
    ...
    case 1: // SiO2
        n = sqrt(
            SiO2_n0 +
            (SiO2_B1 * lambdaum * lambdaum) / (lambdaum * lambdaum - SiO2_C1) +
            (SiO2_B2 * lambdaum * lambdaum) / (lambdaum * lambdaum - SiO2_C2));
        break;

    case 2: // Si
        n = sqrt(1.0 +
                 (Si_B1 * lambdaum * lambdaum) /
                     (lambdaum * lambdaum - Si_C1 * Si_C1) +
                 (Si_B2 * lambdaum * lambdaum) /
                     (lambdaum * lambdaum - Si_C2 * Si_C2) +
                 (Si_B3 * lambdaum * lambdaum) /
                     (lambdaum * lambdaum - Si_C3 * Si_C3));
        break;
    ...
    case 3: // PMGI resist
        ...
        {
            printf(
                "ERROR : PMGI refractive index data does not exist past "
                "1000nm\n");
            exit(0);
        }
        for (i = 0; i < 400; i++)
    ...
    case 4: // PMMA resist
        ...
        {
            printf(
                "ERROR : PMMA refractive index data does not exist past "
                "1000nm\n");
            exit(0);
        }
    ...
    case 5: // N2
        ...
        smb3 = 0.0687966;
        n    = 1.0 +
            (sma1 * lambdaum * lambdaum) / (lambdaum * lambdaum - smb1 * smb1) +
            (sma2 * lambdaum * lambdaum) / (lambdaum * lambdaum - smb2 * smb2) +
            (sma3 * lambdaum * lambdaum) /
                (lambdaum * lambdaum - smb3 * smb3); // Guyon 2016
    ...
    case 6: // O2
        n = 1.0 + 1e-8 * (15532.45 +
                          456402.97 / (50.0 - 1.0 / (lambdaum * lambdaum)));
    ...
    case 7: // Ar
        n = 1.0 +
            1.0e-8 * (6322.05 +
                      2811641.7 / (144.0 - 1.0 / (lambdaum * lambdaum))); // [1]
    ...
    case 10: // H2O vapor
        ...
        w3 = 0.004028;
        n  = 1.0 + 1.0e-8 * 1.022 *
                      (w0 + w1 / lambdaum / lambdaum +
                       w2 / lambdaum / lambdaum / lambdaum / lambdaum +
                       w3 / lambdaum / lambdaum / lambdaum / lambdaum /
                           lambdaum / lambdaum);
    ...
    case 11: // CO2
        n = 1.0 + 0.06991 / (166.175 - 1.0 / lambdaum / lambdaum) +
            0.00144720 / (79.609 - 1.0 / lambdaum / lambdaum) +
    ...
    // 0 C
    case 12: // Ne
        n = 1.0 + 0.012055 * (0.1063 * lambdaum * lambdaum /
                                  (184.661 * lambdaum * lambdaum - 1.0) +
                              1.8290 * lambdaum * lambdaum /
                                  (376.840 * lambdaum * lambdaum - 1.0));
        break;
    ...
    case 13: // O (atomic oxygen)
        pol =
            4.5e-30 * (1.0 + 6.2e5 / lambdaa / lambdaa); // polarizability [m3]
    ...
    case 14: // CaF2
        n = sqrt(1.33973 +
                 (CaF2_B1 * lambdaum * lambdaum) /
                     (lambdaum * lambdaum - CaF2_C1 * CaF2_C1) +
                 (CaF2_B2 * lambdaum * lambdaum) /
                     (lambdaum * lambdaum - CaF2_C2 * CaF2_C2) +
                 (CaF2_B3 * lambdaum * lambdaum) /
                     (lambdaum * lambdaum - CaF2_C3 * CaF2_C3));
        break;
...
double OpticsMaterials_pha_lambda(int material, double z, double lambda)
{
    ...
    nair =
        1.0 + PressureRatio *
                  ((5792105.0e-8 / (238.0185 - 1.0 / (lambdaum * lambdaum))) +
                   (167917.0e-8 / (57.362 - 1.0 / (lambdaum * lambdaum))));
